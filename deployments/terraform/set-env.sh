...
echo "${green} Checking for common_tools_config secret update ${reset}"
...
if kubectl get pods &> /dev/null; then
  ...
  if [ $? -eq 0 ]; then
    echo "${green} Updated <common-tools-config> to newer version ${reset}"
    terraform apply -var-file=common_tools.tfvars  -target=kubernetes_secret.common_tools_config -auto-approve
  fi
else
  echo "${red}Cluster is not up and running skipping update <common-tools-config> update ${reset}"
fi
